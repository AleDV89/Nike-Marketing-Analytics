...
# ---------------------Tabs----------------------#
tab0, tab1, tab2, tab3, tab4 =st.tabs(["About Our Data", "Sales Prediction", "New SQL Database", "Power Bi","Conclusion"])
...
with tab2:
# Configurations
# Función para cargar el archivo SQL
    def load_sql_file(file):
        with open(file, 'r') as f:
         sql_query = f.read()
        return sql_query

# Función para mostrar un DataFrame en Streamlit
    def show_dataframe(df):
        st.write(df)

# Función para ejecutar una consulta en un DataFrame
    def query_dataframe(df, query):
        result = df.query(query)
        return result

    def main():
        st.title("SQL file query in Streamlit")
    ...
    if uploaded_file is not None:
        ...
        # Mostrar contenido del archivo SQL
        st.write("Content of the SQL file:")
        st.code(file_contents, language='sql')
...
with tab3: 
    ...
    def generar_iframe():
        codigo_insercion = """
        <iframe title="Nike" width="100%" height="600" 
        src="https://app.fabric.microsoft.com/view?r=eyJrIjoiNTAxODdjMzctMDRkOS00YTM1LWJkYjMtYjYzNGQwZDIyZDVmIiwidCI6IjhhZWJkZGI2LTM0MTgtNDNhMS1hMjU1LWI5NjQxODZlY2M2NCIsImMiOjl9" 
        frameborder="0" allowFullScreen="true"></iframe>
        """
        return codigo_insercion

# Título en la barra lateral
    st.sidebar.title("Power BI Data")

# Casilla de verificación para mostrar el panel interactivo
    mostrar_panel_principal = st.sidebar.checkbox("Show Power BI Panel")

# Si se marca la casilla de verificación, mostrar el panel interactivo
    if mostrar_panel_principal:
    # Generar y mostrar el iframe
        st.write(generar_iframe(), unsafe_allow_html=True)
